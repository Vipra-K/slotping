...
export class AppointmentService{
 ...
 private dayRange(date?:string){const start=new Date(date?date+'T00:00:00':new Date().toISOString().slice(0,10));if(Number.isNaN(start.getTime()))throw new BadRequestException('Invalid date');const end=new Date(start);end.setDate(end.getDate()+1);return [start,end] as const}
 async create(businessId:string,input:{customerName:string;customerPhone:string;service:string;appointmentAt:string}){
  const appointmentAt=new Date(input.appointmentAt);if(Number.isNaN(appointmentAt.getTime()))throw new BadRequestException('Invalid appointment date');
  if(appointmentAt.getTime()<Date.now())throw new BadRequestException('Appointment must be in the future');
  ...
  const appointment=await this.appointments.save(this.appointments.create({businessId,customerId:customer.id,service:input.service.trim(),appointmentAt,status:'confirmed',reminderSent:false}));
  const log=await this.whatsapp.sendConfirmation(appointment);
  if(log.status==='failed')console.warn('Confirmation failed; appointment remains saved');
  return this.appointments.findOne({where:{id:appointment.id},relations:{customer:true}}) as Promise<Appointment>;
 }
